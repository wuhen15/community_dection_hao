...
def __cover_entropy(cover, allNodes):  # cover is a list of set, no com ID
    ...
    for com in cover:
        ...
        allEntr.append(
            sp.stats.entropy([fractionIn, 1 - fractionIn], base=logBase))
...
def __com_pair_conditional_entropy(cl, clKnown, allNodes):  # cl1,cl2, snapshot_communities (set of nodes)
    ...
    a = len(set(set(allNodes).difference(set(cl))).difference(
        set(clKnown))) / nbNodes
    ...
    if __partial_entropy_a_proba(a) + __partial_entropy_a_proba(
            d) > __partial_entropy_a_proba(b) + __partial_entropy_a_proba(c):
        entropyKnown = sp.stats.entropy(
            [len(clKnown) / nbNodes, 1 - len(clKnown) / nbNodes], base=logBase)
        conditionalEntropy = sp.stats.entropy([a, b, c, d],
                                              base=logBase) - entropyKnown
...
def __cover_conditional_entropy(
        cover,
        coverRef,
        allNodes,
        normalized=False):  # cover and coverRef and list of set
    X = cover
    Y = coverRef
    ...
    # print(coverRef)
    for com in cover:
        matches = [(com2, __com_pair_conditional_entropy(com, com2, allNodes))
                   for com2 in coverRef]
        bestMatch = min(matches, key=lambda c: c[1])
        HXY_part = bestMatch[1]
        if normalized:
            HX = __partial_entropy_a_proba(
                len(com) / len(allNodes)) + __partial_entropy_a_proba(
...
def onmi(cover,
    ...
    if variant == "LFK":
        HXY = __cover_conditional_entropy(cover,
                                          coverRef,
                                          allNodes,
                                          normalized=True)
        HYX = __cover_conditional_entropy(coverRef,
                                          cover,
                                          allNodes,
                                          normalized=True)
    else:
